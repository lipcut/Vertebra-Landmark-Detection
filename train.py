    ...
    def train_network(self, args):
        ...
        for epoch in range(1, args.num_epoch + 1):
            ...
            epoch_loss = self.run_epoch(
                phase="train", data_loader=dsets_loader["train"], criterion=criterion
            )
            train_loss.append(epoch_loss)
            scheduler.step()

            epoch_loss = self.run_epoch(
                phase="val", data_loader=dsets_loader["val"], criterion=criterion
            )
            val_loss.append(epoch_loss)

            np.savetxt(
                os.path.join(save_path, "train_loss.txt"), train_loss, fmt="%.6f"
            )
            ...
            if len(val_loss) > 1:
                if val_loss[-1] < np.min(val_loss[:-1]):
                    self.save_model(
                        os.path.join(save_path, "model_last.pth"), epoch, self.model
                    )
